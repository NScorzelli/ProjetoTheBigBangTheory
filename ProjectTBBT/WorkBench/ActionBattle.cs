...
namespace WorkBench
{
    class ActionBattle
    {
        ...
        public Score Jogar(Score score)
        {
            ...
            {
                options[0] = new Regex("[Pp](r)?ed((r)?a)?", RegexOptions.IgnoreCase); // Pedra
                options[1] = new Regex("[Pp]ap(e[ul])?", RegexOptions.IgnoreCase); // Papel
                options[2] = new Regex("[Tt][ei]so((u)?ra)?", RegexOptions.IgnoreCase); // Tesoura
                options[3] = new Regex("([Ee])?[Ss]p[ou]((qu)?([ck])?([ck])?(e)?)?", RegexOptions.IgnoreCase); // Spock
                options[4] = new Regex("[Ll]a(r)?g(r)?a((r)?t[ou])?", RegexOptions.IgnoreCase); // Lagarto
            }
        ...
        public void SheldonCorrection(string playerPicked, int playerChoose)
        {
        ...
        public Score ReturnLuta(Score score)
        {
        ...
        public int GetOpcoes(Regex[] options, string playerPicked)
        {
        ...
        public int CompararSelecoes(Regex[] opcoes, string playerPicked)
        {
        ...
        public Score Lutar(int sheldonChoose, int playerChoose, Score item)
        {
            var score = item;
        ...
        public void DeclararResultado(Score score)
        {
